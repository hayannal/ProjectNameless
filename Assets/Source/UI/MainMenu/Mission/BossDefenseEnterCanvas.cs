...
public class BossDefenseEnterCanvas : MonoBehaviour
{
	public static BossDefenseEnterCanvas instance;

	const int SELECT_MAX = 8;
	public static int MINIMUM_COUNT = 5;
	...
	StageTableData _bossDefenseStageTableData;
	ObscuredInt _selectedDifficulty;
	ObscuredInt _selectableMaxDifficulty;
	void RefreshInfo()
	{
		int clearLevel = SubMissionData.instance.bossDefenseClearLevel;

		// 최대 가능 레벨은 클리어 한거보다 하나 더 위다.
		_selectableMaxDifficulty = clearLevel + 1;

		// 그래도 Max를 넘을 순 없다.
		if (_selectableMaxDifficulty > BattleInstanceManager.instance.GetCachedGlobalConstantInt("MaxBossDefense"))
			_selectableMaxDifficulty = BattleInstanceManager.instance.GetCachedGlobalConstantInt("MaxBossDefense");

		// 라스트가 있으면 라스트를 부르고 없으면 첫번째인 1로 설정.
		_selectedDifficulty = SubMissionData.instance.bossDefenseSelectedLevel;
	...
	{
		// 가격
		int price = BattleInstanceManager.instance.GetCachedGlobalConstantInt("MissionEnergyBossDefense");
	...
	{
		TooltipCanvas.Show(true, TooltipCanvas.eDirection.Bottom, UIString.instance.GetString("MissionUI_BossDefenseTitleMore"), 300, titleTextTransform, new Vector2(0.0f, -35.0f));
	}
	...
	void RefreshDifficulty()
	{
		...
		newObject.SetActive(_selectedDifficulty == _selectableMaxDifficulty && _selectedDifficulty > SubMissionData.instance.bossDefenseClearLevel);

		// 현재 선택된 난이도에 따른 보상을 보여준다.
		MissionModeTableData missionModeTableData = TableDataManager.instance.FindMissionModeTableData((int)SubMissionData.eSubMissionType.BossDefense, _selectedDifficulty);
	...
	public void OnClickYesButton()
	{
		...
		{
			ToastCanvas.instance.ShowToast(UIString.instance.GetString("MissionUI_BossDefenseSelectMinimum"), 2.0f);
			return;
		}
	...
	IEnumerator<float> BattleMoveProcess()
	{
		...
		UIInstanceManager.instance.ShowCanvasAsync("BossDefenseMissionCanvas", () =>
		{
	...
	void RecordLastCharacterList()
	{
		...
		ObscuredPrefs.SetString(string.Format("_bdEnterCanvas_{0}", PlayFabApiManager.instance.playFabId), value);
	}

	List<string> GetCachedLastCharacterList()
	{
		string cachedLastCharacterList = ObscuredPrefs.GetString(string.Format("_bdEnterCanvas_{0}", PlayFabApiManager.instance.playFabId));
