...
public class BossDefenseMissionCanvas : MonoBehaviour
{
	public static BossDefenseMissionCanvas instance;

	public GameObject bossDefenseMissionPrefab;
	...
	void OnEnable()
	{
		...
		if (_groundTransform == null)
			_groundTransform = BattleInstanceManager.instance.GetCachedObject(bossDefenseMissionPrefab, StageManager.instance.GetSafeWorldOffset(), Quaternion.identity).transform;
		else
	...
	IEnumerator<float> AutoPositionProcess()
	{
		...
		for (int i = 0; i < _listCharacterToggleButton.Count; ++i)
		{
			...
			BossDefenseMissionGround.instance.OnClickBox(lastPositionIndex);
		...
		for (int i = 0; i < listRemainIndex.Count; ++i)
		{
			...
			int emptyPositionIndex = BossDefenseMissionGround.instance.GetEmptyWorldButtonIndex();
			...
			BossDefenseMissionGround.instance.OnClickBox(emptyPositionIndex);
			yield return Timing.WaitForSeconds(0.5f);
		}
	...
	{
		List<string> listSelectedActorId = BossDefenseEnterCanvas.instance.listSelectedActorId;
	...
	public void OnSpawnActor(int positionIndex)
	{
		...
		if (_spawnedCount == _listCharacterToggleButton.Count)
		{
			...
			BossDefenseMissionGround.instance.OnFinishSelect();
			return;
		}
	...
	{
		MissionModeTableData missionModeTableData = TableDataManager.instance.FindMissionModeTableData((int)SubMissionData.eSubMissionType.BossDefense, selectedDifficulty);
	...
	void RecordLastPositionInfo()
	{
		...
		ObscuredPrefs.SetString(string.Format("_bdPosition_{0}", PlayFabApiManager.instance.playFabId), value);
	}

	Dictionary<string, string> GetCachedLastPositionInfo()
	{
		string cachedLastPositionInfo = ObscuredPrefs.GetString(string.Format("_bdPosition_{0}", PlayFabApiManager.instance.playFabId));
