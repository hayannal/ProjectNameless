...
public class GoldDefenseMissionCanvas : MonoBehaviour
{
	public static GoldDefenseMissionCanvas instance;

	public GameObject goldDefenseMissionPrefab;
	...
	void OnEnable()
	{
		...
		if (_groundTransform == null)
			_groundTransform = BattleInstanceManager.instance.GetCachedObject(goldDefenseMissionPrefab, StageManager.instance.GetSafeWorldOffset(), Quaternion.identity).transform;
		else
	...
	IEnumerator<float> AutoPositionProcess()
	{
		...
		for (int i = 0; i < _listCharacterToggleButton.Count; ++i)
		{
			...
			GoldDefenseMissionGround.instance.OnClickBox(lastPositionIndex);
		...
		for (int i = 0; i < listRemainIndex.Count; ++i)
		{
			...
			int emptyPositionIndex = GoldDefenseMissionGround.instance.GetEmptyWorldButtonIndex();
			...
			GoldDefenseMissionGround.instance.OnClickBox(emptyPositionIndex);
			yield return Timing.WaitForSeconds(0.5f);
		}
	...
	{
		List<string> listSelectedActorId = GoldDefenseEnterCanvas.instance.listSelectedActorId;
	...
	public void OnSpawnActor(int positionIndex)
	{
		...
		if (_spawnedCount == _listCharacterToggleButton.Count)
		{
			...
			GoldDefenseMissionGround.instance.OnFinishSelect();
			return;
		}
	...
	{
		MissionModeTableData missionModeTableData = TableDataManager.instance.FindMissionModeTableData((int)SubMissionData.eSubMissionType.GoldDefense, selectedDifficulty);
