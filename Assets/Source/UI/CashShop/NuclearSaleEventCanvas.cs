...
public class NuclearSaleEventCanvas : SimpleCashEventCanvas
{
	public static NuclearSaleEventCanvas instance;
	...
	void Start()
	{
		...
		string id = string.Format("{0}_nuclearsale", tableId);
	...
	public static void ExternalRequestServerPacket(Product product)
	{
...
#endif
		{
			...
			if (instance != null)
				shopProductTableData = TableDataManager.instance.FindShopProductTableData(string.Format("{0}_nuclearsale", instance.cashEventId));
			else
