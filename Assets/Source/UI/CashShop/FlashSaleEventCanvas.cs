...
public class FlashSaleEventCanvas : SimpleCashEventCanvas
{
	public static FlashSaleEventCanvas instance;
	...
	void Start()
	{
		...
		string id = string.Format("{0}_flashsale", tableId);
	...
	public static void ExternalRequestServerPacket(Product product)
	{
...
#endif
		{
			...
			if (instance != null)
				shopProductTableData = TableDataManager.instance.FindShopProductTableData(string.Format("{0}_flashsale", instance.cashEventId));
			else
